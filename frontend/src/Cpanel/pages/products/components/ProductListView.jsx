...
import { FiExternalLink, FiEdit2, FiTrash2, FiEye, FiChevronLeft, FiChevronRight } from 'react-icons/fi';
...
function ImageCarousel({ images = [], name }) {
  ...
  return (
    <div className="group relative w-full max-w-[260px] h-[180px] rounded-xl bg-light-bg dark:bg-dark-hover border border-primary/10 overflow-hidden flex items-center justify-center p-2">
      {hasImages ? (
        <>
    ...
    <div className="space-y-4 -mt-2">
      <div className="p-1 space-y-4">
        {!hideHeading ? (
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-3">
            <div>
              ...
              <p className="text-sm text-text-secondary dark:text-text-light/70">Preview customer-style list.</p>
            </div>
          </div>
        ) : null}
        ...
        <div className="flex flex-col gap-4">
          {loading ? (
            <div className="text-center text-text-secondary dark:text-text-light/70 py-6">Loading products...</div>
          ) : null}
          {!loading &&
            products.map((product) => {
              ...
              return (
                <div
                  key={product._id}
                  className="rounded-2xl border border-primary/10 bg-light-card dark:bg-dark-card shadow-soft dark:shadow-strong overflow-hidden flex flex-col md:flex-row md:items-center gap-4 p-3 md:p-4"
                >
                  <ImageCarousel images={product.images || []} name={product.name} />

                  <div className="flex-1 min-w-0 space-y-2">
                    <div className="text-xs font-semibold text-text-secondary dark:text-text-light/70 uppercase flex items-center gap-2">
                      {product.category?.name || 'Uncategorized'}
                      <span className="text-text-secondary/70">•</span>
                      <span className="text-text-secondary/70">{product.stock ?? 0} in stock</span>
                    </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      {product.regularPrice ? (
                        <div className="text-sm line-through text-text-secondary/70">
                          ${product.regularPrice.toFixed ? product.regularPrice.toFixed(2) : product.regularPrice}
                        </div>
                      ) : null}
                    ...
                    <div className="text-xs font-semibold text-text-secondary dark:text-text-light/70">
                      Code: {product.productCode || '—'} • Unit profit: ${unitProfit.toLocaleString()}
                    </div>
                  </div>
